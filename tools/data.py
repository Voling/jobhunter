...
def search(keyw, loca, rad, lat_input, lng_input):
    ...
    for i in range(len(s)):
        ...
        dct["Title"] = json.loads(s[i])["title"]
        dct["Location"] = json.loads(s[i])["location"]

        #print("Company: " + json.loads(s[i])["company"])
        dct["Company"] = json.loads(s[i])["company"]
...
#data = search("AWS", coor_to_zip.get_zip(33.6396288,-117.8435584), 5, 33.6396288,-117.8435584)
data = search("software engineer", coor_to_zip.get_zip(33.6491, -117.8474), 10, 33.6471, -117.8424)
'''
for d in data:
    print(d)
    print(" ")
